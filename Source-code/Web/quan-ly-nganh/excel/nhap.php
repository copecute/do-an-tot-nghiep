...
// lấy danh sách khoa để map tên sang id
$dsKhoa = [];
try {
    ...
    if (isset($_FILES['file_excel']) && $_FILES['file_excel']['error'] == 0) {
        ...
        try {
            ...
            for ($i = 1; $i < count($rows); $i++) { // bắt đầu từ dòng 2
                $row = $rows[$i];
                $tenNganh = trim($row[1] ?? '');
                $tenKhoa = mb_strtolower(trim($row[2] ?? ''));
                $skipReason = '';
                if (!$tenNganh) $skipReason = 'Thiếu tên ngành';
                if (!$tenKhoa) $skipReason = 'Thiếu tên khoa';
                $khoaId = $dsKhoa[$tenKhoa] ?? null;
                if (!$khoaId && !$skipReason) $skipReason = 'Tên khoa không khớp hệ thống';
                if ($skipReason) {
                    $skipped++;
            ...
            $pdo->commit();
            $success = 'Đã nhập thành công ' . $count . ' ngành!';
            if ($skipped > 0) {
...
<nav aria-label="breadcrumb" class="mx-4 my-3">
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item"><a href="/quan-ly-nganh">Quản Lý Ngành</a></li>
...
<div class="container-fluid">
    <div class="card shadow-sm mb-4">
        <div class="card-header bg-light d-flex align-items-center">
            ...
            <span class="fw-bold fs-4">Công Cụ Nhập/Xuất Ngành</span>
        </div>
        <div class="card-body">
            <div class="row">
                <div class="col-md-6 mb-3">
                    <div class="card h-100">
                        ...
                        <div class="card-body">
                            <div class="mb-2">Nhập dữ liệu ngành từ file Excel.</div>
                            <form method="post" enctype="multipart/form-data">
                                ...
                                <div class="d-flex gap-2">
                                    <a href="/quan-ly-nganh/excel/mau.php" class="btn btn-success" id="btnTaiMauNhap"><i class="fas fa-download me-1"></i> Tải Mẫu Nhập</a>
                ...
                <div class="col-md-6 mb-3">
                    <div class="card h-100">
                        ...
                        <div class="card-body">
                            <div class="mb-3">Xuất dữ liệu ngành ra file Excel.</div>
                            <a href="/quan-ly-nganh/excel/xuat.php" class="btn btn-success" id="btnXuatDuLieu"><i class="fas fa-download me-1"></i> Xuất Dữ Liệu</a>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <div class="card mt-3">
                        ...
                        <div class="card-body">
                            <ul class="mb-0">
                                ...
                                <li>Phải nhập đúng tên khoa đã có trong hệ thống.</li>
                                <li>Mỗi dòng là một ngành, không để trống cột nào.</li>
...
const formNhapNganhExcel = document.querySelector('form[enctype="multipart/form-data"]');
const btnNhapNganhExcel = document.getElementById('btnNhapDuLieu');
if (formNhapNganhExcel && btnNhapNganhExcel) {
    formNhapNganhExcel.addEventListener('submit', function(e) {
        btnNhapNganhExcel.disabled = true;
        btnNhapNganhExcel.innerHTML = `<span class=\"spinner-border spinner-border-sm\" aria-hidden=\"true\"></span> <span class=\"visually-hidden\" role=\"status\">Loading...</span>`;
    });
}
</script>
