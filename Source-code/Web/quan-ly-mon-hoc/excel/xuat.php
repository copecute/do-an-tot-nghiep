...
header('Content-Disposition: attachment;filename="danh_sach_mon_hoc.xlsx"');
...
$sheet->setCellValue('A1', 'DANH SÁCH MÔN HỌC');
...
try {
    $stmt = $pdo->query('
        SELECT m.tenMonHoc, n.tenNganh, k.tenKhoa
        FROM monHoc m
    ...
    foreach ($rows as $row) {
        ...
        $sheet->setCellValue('B'.$rowNum, $row['tenMonHoc']);
        $sheet->setCellValue('C'.$rowNum, $row['tenNganh']);
        $sheet->setCellValue('D'.$rowNum, $row['tenKhoa']);
        $rowNum++;
    }
} catch (Exception $e) {
...
$sheet->getStyle('A5:D'.$lastRow)->applyFromArray($borderStyle);

// Căn trái dữ liệu
$sheet->getStyle('A6:D'.$lastRow)->getAlignment()->setHorizontal(Alignment::HORIZONTAL_LEFT);
